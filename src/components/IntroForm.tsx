  ...
  return (
    <section className="screen intro fade-in" aria-labelledby="app-title">
      ...
      <form className="ritual-card hanji" onSubmit={submit}>
        ...
        <label>이름<input value={name} onChange={(e) => setName(e.target.value)} placeholder="이름을 입력하세요" /></label>
        <label>생년월일<input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} /></label>
        <label>태어난 시간<input type="time" value={birthTime} onChange={(e) => setBirthTime(e.target.value)} /></label>
